...
20. 旧 `schemaVersion: 6` の主台帳データを読み込んでも、`benefits` が `[]` で補完されて表示が壊れないことを確認
21. `/` の `特典期限アラート` に、未受取かつ受取期限日が30日以内または超過の特典が表示されることを確認
22. `/` の `今後のアクション予定` に、予定日が60日以内または超過の利用中 / 解約予定回線が表示されることを確認
23. `/` の `番号・無料オプション期限` に、MNP予約番号期限と無料オプション期限が3日以内または超過の回線が表示されることを確認
24. `利用中` または `解約予定` の回線詳細に `解約可能推奨日` が表示され、`解約済み` / `MNP転出済み` では表示されないことを確認
25. `利用中` または `解約予定` の回線に `通 / 話 / S` の利用実績バッジが表示され、実績ゼロの回線では `利用実績なし` が表示されることを確認
26. `npx tsc --noEmit` でビルドエラーがないことを確認
27. PR では GitHub Actions の `CI` workflow が成功していることを確認
