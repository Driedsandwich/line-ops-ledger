...
18. `/` の `今後のアクション予定` に、予定日が60日以内または超過の利用中 / 解約予定回線が表示されることを確認
19. `利用中` または `解約予定` の回線詳細に `解約可能推奨日` が表示され、`解約済み` / `MNP転出済み` では表示されないことを確認
20. `npx tsc --noEmit` でビルドエラーがないことを確認
21. PR では GitHub Actions の `CI` workflow が成功していることを確認
