...
9. `固定候補をクリア` / `非表示候補をクリア` / `候補管理を初期化` が動き、再読み込み後も維持されることを確認
10. `/settings/storage`、`/settings/backup`、`/settings/notifications`、`/settings/activity-types` が直接開けることを確認
11. `npx tsc --noEmit` でビルドエラーがないことを確認
12. PR では GitHub Actions の `CI` workflow が成功していることを確認
