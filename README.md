...
10. custom 候補の `現在の文言で更新` が動き、pin 済み候補でも更新文言に追従することを確認
11. `固定候補をクリア` / `非表示候補をクリア` / `候補管理を初期化` が動き、再読み込み後も維持されることを確認
12. `/settings/storage`、`/settings/backup`、`/settings/notifications`、`/settings/activity-types` が直接開けることを確認
13. `npx tsc --noEmit` でビルドエラーがないことを確認
14. PR では GitHub Actions の `CI` workflow が成功していることを確認
