...
- 復元済み活動種別の履歴入力反映 E2E を追加
  - 統合バックアップ復元後、復元されたカスタム活動種別を `/lines/history` の活動種別 select で選択できることを確認する
  - その活動種別で履歴を保存し、タイムラインに活動種別と活動メモが残ることを mobile / desktop の両 viewport で固定する
...
- `docs/03_status.md` の現在地を分割
  - 1行に集約していた PR 履歴を、基盤更新 / 主な E2E 拡張 / 直近の追加確認へ分ける
  - 既存事実の再配置に留め、実装仕様や検証済み範囲の意味は変えない
