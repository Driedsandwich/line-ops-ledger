...
## 2026-04-04
- Issue #199: `/lines/history` に要点帯とクイック確認を追加
  - 上部に `履歴の要点 / クイック確認` を追加し、履歴件数・活動ログ・表示中の関連回線を一目で見られるようにした
  - `フォームへ` / `タイムラインへ` / `回線一覧へ` / `バックアップへ` のクイック導線を上部帯に集約した
  - `quickActivity`、下書き復元、候補管理、エクスポート/インポートは維持したまま、視認性だけを調整した
