...
1. 通知設定の有効/無効反映 E2E PR の `check` / `build` / `test:e2e` / `audit` と GitHub Actions を通し、Dashboard / Lines の通知表示に回帰がないことを確認する
2. `npm audit --audit-level=low` と `npm outdated --depth=0` を継続監視する
3. 次の品質改善は、通知期限 window 変更後の対象件数変化や、Dashboard / Lines 間の filter URL 同期確認など、未自動化の導線を小さく追加する
