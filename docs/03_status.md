...
1. 旧統合バックアップ互換 E2E PR の `check` / `build` / `test:e2e` / `audit` と GitHub Actions を通し、旧ファイル復元に回帰がないことを確認する
2. `npm audit --audit-level=low` と `npm outdated --depth=0` を継続監視する
3. 次の品質改善は、通知設定変更後の drilldown や主台帳単体 backup file の互換 E2E など、未自動化のユーザー導線を小さく追加する
