...
1. 主台帳単体バックアップ互換 E2E PR の `check` / `build` / `test:e2e` / `audit` と GitHub Actions を通し、旧ファイル復元に回帰がないことを確認する
