...
1. 活動メモ候補の管理状態リセット E2E PR の `check` / `build` / `test:e2e` / `audit` と GitHub Actions を通し、候補管理の reset 導線に回帰がないことを確認する
2. `npm audit --audit-level=low` と `npm outdated --depth=0` を継続監視する
3. 次の品質改善は、通知理由別 filter の追加クリック確認など、未自動化のユーザー導線を小さく追加する
