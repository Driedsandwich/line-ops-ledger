...
1. 表示中一括選択 / 解除 E2E PR の `check` / `build` / `test:e2e` / `audit` と GitHub Actions を通し、フィルタ中の可視行選択に回帰がないことを確認する
2. `npm audit --audit-level=low` と `npm outdated --depth=0` を継続監視する
3. 次の品質改善は、バックアップ復元後の候補設定保持や通知設定変更後の drilldown など、未自動化のユーザー導線を小さく追加する
