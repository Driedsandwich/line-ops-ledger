...
1. Dashboard から Lines への sort URL 同期 E2E PR の `check` / `build` / `test:e2e` / `audit` と GitHub Actions を通し、URL と Lines UI の初期状態に回帰がないことを確認する
2. `npm audit --audit-level=low` と `npm outdated --depth=0` を継続監視する
3. 次の品質改善は、通知理由別件数の境界ケースや、Dashboard からの他 drilldown の表示状態確認など、未自動化の導線を小さく追加する
