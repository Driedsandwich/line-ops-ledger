...
1. サンプルデータ dashboard E2E PR の `check` / `build` / `test:e2e` / `audit` と GitHub Actions を通し、空状態からのサンプル投入と `historyIntent` drilldown に回帰がないことを確認する
2. `npm audit --audit-level=low` と `npm outdated --depth=0` を継続監視する
3. 次の品質改善は、`/lines` 側の URL drilldown（`openDraft` / `focusSection` / `usagePriority`）や backup restore 後の追加編集など、未自動化のユーザー導線を小さく追加する
