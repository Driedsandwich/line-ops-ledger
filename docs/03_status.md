...
1. 活動メモ候補設定を含む統合バックアップ PR の `check` / `build` / `test:e2e` / `audit` と GitHub Actions を通し、復元後の候補設定に回帰がないことを確認する
2. `npm audit --audit-level=low` と `npm outdated --depth=0` を継続監視する
3. 次の品質改善は、通知設定変更後の drilldown や旧 backup file の互換 E2E など、未自動化のユーザー導線を小さく追加する
