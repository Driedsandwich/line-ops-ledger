...
- 現在地: 利用実績種別カードから `契約中のみ` 付きで `/lines` を開ける導線まで `main` 反映済み
...
1. 受取済み特典一覧から `/lines` 側の特典管理位置へ辿りやすくする導線強化
2. 光回線カードから `/lines` 側の光回線詳細位置へ辿りやすくする導線強化
3. 利用実績種別カードから不足種別に応じた `/lines` フィルタへ辿りやすくする導線強化
