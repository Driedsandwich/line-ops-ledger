...
1. 復元済み活動種別の履歴入力反映 E2E PR の `check` / `build` / `test:e2e` / `audit` と GitHub Actions を通し、復元後の入力導線に回帰がないことを確認する
2. `npm audit --audit-level=low` と `npm outdated --depth=0` を継続監視する
3. 次の品質改善は、通知理由別 filter の追加クリック確認や活動メモ候補 reset 系の追加確認など、未自動化のユーザー導線を小さく追加する
