...
- 現在地: 収支サマリーから受取済み特典の回線一覧へ辿れる導線まで `main` 反映済み
...
- `収支サマリー` から、受取済み特典がある回線一覧と `/lines` 導線を表示
...
1. 光回線の回線詳細導線強化（ダッシュボード補助カードから `/lines` へ辿れる導線）の追加
2. 利用実績種別カードから `/lines?sort=latestActivityAsc` へ辿る巡回導線のフィルタ強化
3. 受取済み特典一覧から `/lines` 側の特典管理位置へ辿りやすくする導線強化
