...
1. 通知理由 filter 中の表示行一致 E2E PR の `check` / `build` / `test:e2e` / `audit` と GitHub Actions を通し、通知対象のみ filter と通知理由 filter の組み合わせに回帰がないことを確認する
2. `npm audit --audit-level=low` と `npm outdated --depth=0` を継続監視する
3. 次の品質改善は、Dashboard / Lines 間の filter URL 同期確認や、通知理由別件数の境界ケースなど、未自動化の導線を小さく追加する
