...
1. 通知期限 window 変更後の対象件数連動 E2E PR の `check` / `build` / `test:e2e` / `audit` と GitHub Actions を通し、Dashboard / Lines の通知対象件数に回帰がないことを確認する
2. `npm audit --audit-level=low` と `npm outdated --depth=0` を継続監視する
3. 次の品質改善は、Dashboard / Lines 間の filter URL 同期確認や、通知理由ごとの行表示確認など、未自動化の導線を小さく追加する
