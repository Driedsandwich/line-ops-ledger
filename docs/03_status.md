...
- 現在地: 利用実績種別のダッシュボード巡回カードまで `main` 反映済み
...
1. 受取済み特典の回線詳細導線強化（収支サマリーから `/lines` へ辿れる導線）の追加
2. 光回線の回線詳細導線強化（ダッシュボード補助カードから `/lines` へ辿れる導線）の追加
3. 利用実績種別カードから `/lines?sort=latestActivityAsc` へ辿る巡回導線のフィルタ強化
