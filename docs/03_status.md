...
1. 活動種別設定を含む統合バックアップ PR の `check` / `build` / `test:e2e` / `audit` と GitHub Actions を通し、復元後の活動種別設定に回帰がないことを確認する
2. `npm audit --audit-level=low` と `npm outdated --depth=0` を継続監視する
3. 次の品質改善は、活動種別設定の履歴入力反映確認や通知理由別 filter の追加クリック確認など、未自動化のユーザー導線を小さく追加する
