...
- 統合バックアップエクスポート・インポート（主台帳＋履歴＋活動メモ候補設定）
...
1. 通知設定変更後の通知対象 drilldown E2E PR の `check` / `build` / `test:e2e` / `audit` と GitHub Actions を通し、通知導線に回帰がないことを確認する
2. `npm audit --audit-level=low` と `npm outdated --depth=0` を継続監視する
3. 次の品質改善は、通知設定変更後の drilldown や活動種別設定の backup 対象化検討など、未自動化のユーザー導線を小さく追加する
