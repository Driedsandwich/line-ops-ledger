...
1. Dashboard 未使用 panel helper 整理 PR の `check` / `build` / `test:e2e` / `audit` と GitHub Actions を通し、現行 Dashboard 表示に回帰がないことを確認する
2. `npm audit --audit-level=low` と `npm outdated --depth=0` を継続監視する
3. 次の品質改善は、設定画面の通知方針変更後の dashboard / lines 表示確認や、Dashboard / Lines 間の filter URL 同期確認など、未自動化の導線を小さく追加する
