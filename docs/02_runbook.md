...
12. `ライト表示` / `ダーク表示` と `ナビを閉じる` / `ナビを開く` が動き、再読み込み後も選択状態が残ることを確認する
13. ブラウザのキャッシュが古いときは、開発サーバー再起動で最新表示を取り込む
...
- 主台帳（`/lines`）
  ...
  - [ ] 検索パネルの `通知対象のみ` / `契約中のみ` が横並びの switch-like control として表示され、checkbox が不自然な位置に浮いて見えない
...
- [ ] 高さの低い viewport でも、左側サイドバー（サイドパネル）が縦スクロールできる
  - [ ] 左側サイドバーを閉じる / 開く操作ができ、閉じた状態でも本文が左端ボタンと重ならない
  - [ ] light / dark theme を切り替えられ、フォーム・カード・サイドバーの色が同じテーマに揃う
  - [ ] 検索・絞り込み・並び替えが動く
